  ...
  return (
    ...
    >
      ...
      <section style={{ padding: "96px 24px 72px", textAlign: "center" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <p style={{ maxWidth: 700, margin: "24px auto 0", color: "#8885a8", lineHeight: 1.8 }}>
            Midilli AI brings your landing page and Supabase auth setup into one
            polished flow so visitors can sign up, verify, and get to work faster.
          </p>
            ...
            <>
              ...
              {(loading || imageResult) && (
                ...
                >
                  {loading ? (
                    <>
                      <div style={{ fontSize: 34 }}>Studio</div>
                      <div style={{ color: "#8885a8" }}>Generating preview...</div>
                    </>
                  ) : (
                    <>
                      <div style={{ fontSize: 24, fontWeight: 700 }}>Preview Ready</div>
      ...
      <section id="gallery" style={{ maxWidth: 1100, margin: "0 auto", padding: "110px 24px 0" }}>
        <SectionHead label="Community" title="User Gallery" sub="A cleaner showcase section that keeps the premium landing page feel." />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16, marginTop: 42 }}>
          {galleryItems.map(([label, user, background]) => (
            <div key={label} style={{ borderRadius: 18, overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)" }}>
              <div style={{ minHeight: 260, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 10, background }}>
                <div style={{ fontSize: 14, letterSpacing: 2, textTransform: "uppercase", color: "rgba(255,255,255,0.72)" }}>
                  Showcase
                </div>
      ...
      <section id="pricing" style={{ maxWidth: 1100, margin: "0 auto", padding: "110px 24px 0" }}>
        <SectionHead label="Pricing" title="Plans for Everyone" sub="Membership tiers stay connected to the auth flow with clearer calls to action." />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 18, marginTop: 42 }}>
          ...
          <PricingCard title="Pro" price="$19" credits="500 / mo" featured features={["Private gallery", "Priority queue", "Commercial usage"]} cta={<button style={primaryButton} onClick={() => showToast("Pro checkout sonraki adimda baglanabilir.")}>Go Pro</button>} />
          <PricingCard title="Ultra" price="$49" credits="2000 / mo" features={["4K exports", "API-ready", "Instant queue"]} cta={<button style={secondaryButton} onClick={() => showToast("Ultra plan akisi hazir.")}>Go Ultra</button>} />
        </div>
      </section>

      <section id="credits" style={{ maxWidth: 980, margin: "0 auto", padding: "110px 24px 90px" }}>
        <SectionHead label="Credit Store" title="Buy Credits" sub="One-time packs stay available for quick upgrades in the demo flow." />
      ...
      <footer style={{ textAlign: "center", padding: "0 24px 40px", color: "#8885a8" }}>
        (c) 2026 Midilli AI. All rights reserved.
      </footer>
